import { type TypedDataField, ethers, zeroPadValue } from 'ethers-v6';
...
function getDefaultForBaseType(type: string): any {
  ...
  if (width) return zeroPadValue('0x', Number.parseInt(width));

  if (type.match(/^(u?)int(\d*)$/)) type = 'integer';

  return baseDefaults[type as keyof typeof baseDefaults];
}
