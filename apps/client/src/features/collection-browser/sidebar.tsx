...
import type { TraitValue } from 'lootex/api/endpoints/collection';
...
const PriceRange = ({ collection }: { collection?: LootexCollection }) => {
  ...
  const allowCurrencies = collection?.allowCurrencies || [];
  const defaultSymbol = allowCurrencies?.[0]?.symbol || 'ETH';
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        ...
        <SelectContent>
          {allowCurrencies?.length >= 1 ? (
            allowCurrencies?.map((c) => (
