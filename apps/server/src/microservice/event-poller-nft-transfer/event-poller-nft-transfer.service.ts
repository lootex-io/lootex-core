...
export class EventPollerNftTransferService {
  ...
  // ContractAddress -> Array of {start, end}
  private whitelistRangeConfig: Map<string, { start: number; end: number }[]> = new Map();
  ...
  constructor(
    ...
    const chainId = parseInt(this.configService.get('CHAIN_ID') || '1868');

    // Init Whitelist
    this.chainIdToWhitelistMap.set(chainId, new Set());

    // Load from ENV dynamically based on CHAIN_NAME
    const chainName = (this.configService.get('CHAIN_NAME') || 'soneium').toUpperCase();
    const envVarName = `NFT_POLLER_WHITELIST_${chainName}`;
    const whitelistStr = this.configService.get(envVarName);

    if (whitelistStr) {
      const items = whitelistStr.split(',').map(item => item.trim()).filter(item => item.length > 0);

      items.forEach(item => {
        // Format: ADDRESS:START_ID:END_ID or just ADDRESS
        const parts = item.split(':');
        const address = parts[0].toLowerCase();

        this.chainIdToWhitelistMap.get(chainId).add(address);

        if (parts.length === 3) {
          const startId = parseInt(parts[1]);
          const endId = parseInt(parts[2]);
          if (!isNaN(startId) && !isNaN(endId)) {
            if (!this.whitelistRangeConfig.has(address)) {
              this.whitelistRangeConfig.set(address, []);
            }
            this.whitelistRangeConfig.get(address).push({ start: startId, end: endId });
          }
        }
      });

      this.logger.log(`Loaded ${items.length} whitelisted items for ${chainName} (ChainID: ${chainId}) from ENV ${envVarName}.`);
    } else {
      this.logger.warn(`No whitelist configured for ${chainName} (${envVarName}). Poller will skip all events.`);
    }
  }
  ...
  projects: EventProject[] = [
    ...
    // },
    {
      name: `NFT_Transfer_${(this.configService.get('CHAIN_NAME') || 'Soneium')}`,
      chainId: parseInt(this.configService.get('CHAIN_ID') || '1868'),
      abi: [ERC_721_ABI, ERC_1155_ABI],
      pollingBatch: 10,
    },
    // {
  ...
  async syncWhitelistRanges() {
    this.logger.log('Starting syncWhitelistRanges...');
    const chainId = parseInt(this.configService.get('CHAIN_ID') || '1868');

    for (const [address, ranges] of this.whitelistRangeConfig) {
      for (const range of ranges) {
        // Key to store the number of the last successfully synced token ID
        const progressKey = `poller:range_sync_progress:${chainId}:${address}:${range.start}:${range.end}`;

        // Value will be the Token ID (number)
        const lastSyncedTokenIdStr = await this.cacheService.getCache(progressKey);
        let startFrom = range.start;

        if (lastSyncedTokenIdStr !== null && lastSyncedTokenIdStr !== undefined) {
          const lastSyncedTokenId = parseInt(lastSyncedTokenIdStr as string);

          if (lastSyncedTokenId >= range.end) {
            this.logger.log(
              `Skipping whitelist range for ${address}: ${range.start} to ${range.end} (Fully Synced at ${lastSyncedTokenId})`,
            );
            continue;
          }

          if (lastSyncedTokenId >= range.start) {
            startFrom = lastSyncedTokenId + 1;
            this.logger.log(
              `Resuming whitelist range for ${address}: from ${startFrom} (Last synced: ${lastSyncedTokenId}) to ${range.end}`,
            );
          }
        } else {
          this.logger.log(
            `Syncing whitelist range for ${address}: ${range.start} to ${range.end}`,
          );
        }

        // Pass progressKey so runBatchSync can update it
        this.runBatchSync(address, startFrom, range.end, progressKey).catch(err =>
          this.logger.error(`Error syncing range ${address} ${range.start}-${range.end}: ${err.message}`)
        );
      }
    }
  }

  async runBatchSync(contractAddress: string, start: number, end: number, progressKey: string) {
    const chainId = parseInt(this.configService.get('CHAIN_ID') || '1868');
    const BATCH_SIZE = 50;
    const rpcChain = ChainUtil.chainIdToRpcChain(chainId);
    ...
    try {
      for (let i = start; i <= end; i += BATCH_SIZE) {
        ...
        for (let j = i; j <= batchEnd; j++) {
          tokenIds.push(j.toString());
        }

        this.logger.debug(
          `Syncing batch ${tokenIds[0]} - ${tokenIds[tokenIds.length - 1]} for ${contractAddress}`,
        );
        ...
        const results = await this.rpcService.get(rpcChain, [
          {
        ...
        if (results && results.length > 0) {
          const returnValues = results[0].value;

          for (let k = 0; k < tokenIds.length; k++) {
            const tokenId = tokenIds[k];
            const ownerIdx = k * 2;
            const uriIdx = k * 2 + 1;

            const ownerRes = returnValues[ownerIdx];
            const uriRes = returnValues[uriIdx];

            if (!ownerRes.success || !uriRes.success) {
              // this.logger.warn(`Failed to fetch data for ${contractAddress} #${tokenId}`);
              continue;
            }

            const ownerAddress = ownerRes.returnValues[0].toLowerCase();
            ...
            let asset = await this.assetDao.syncAssetOnChain(assetKey, {
              ...
              syncOwnerShip: false, // We sync manually below
            });
            ...
            if (asset) {
              ...
              // Update/Create AssetAsEthAccount
              const assetEthAccount = await this.assetAsEthAccountRepository.findOne({
                where: {
                  assetId: asset.id,
                  contractId: asset.contractId,
                }
              ...
              if (assetEthAccount) {
                ...
                await this.assetAsEthAccountRepository.create(values);
              }
            }
          }
        }

        // Update Checkpoint: Mark this batch's last token as synced
        // TTL 0 = infinite (or use 24h+ if you prefer expiration)
        await this.cacheService.setCache(progressKey, batchEnd, 0);

        // Sleep slightly to avoid rate limit?
        await new Promise(r => setTimeout(r, 200));
      }

      this.logger.log(`Range Sync Completed: ${progressKey} (End: ${end})`);

    } catch (err) {
      this.logger.error(`runBatchSync failed fatal: ${contractAddress}`, err);
    }
  }
