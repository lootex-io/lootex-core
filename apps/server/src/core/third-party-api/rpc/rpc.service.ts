...
export class RpcService {
  ...
  getMulticall(chainId: number): Multicall {
    ...
    // Check if the requested chain matches the configured chain ID and if a custom address exists
    // Fallback to Soneium specific check if environment variable is missing (to maintain behavior if .env not set, or purely rely on env?)
    // User requested to move to .env, so we rely on .env.
    const configuredChainId = this.configService.get('CHAIN_ID');
    const customAddress = this.configService.get('CHAIN_MULTICALL_ADDRESS');

    if (Number(chainId) === Number(configuredChainId) && customAddress) {
      multiConfig = {
        ...multiConfig,
