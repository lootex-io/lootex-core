...
export class AccountService {
  ...
  async getReferralAccounts(account: Account, limit: number, page: number) {
    if (!account) {
      return {
        accounts: [],
        count: 0,
    ...
    const accountReferral =
      await this.accountReferralRepository.findAndCountAll({
        where: {
    ...
    const accounts = await promise.map(
      accountReferral.rows,
      async (referral) => {
        return {
          category: referral.category,
          username: referral.Referral.username,
          fullname: referral.Referral.fullname,
          avatarUrl: referral.Referral.avatarUrl,
          createdAt: referral.Referral.createdAt,
          badge: referral.Referral.Badge?.name || null,
          avatarDecoration: referral.Referral.AvatarDecoration?.name || null,
          s2BasicCompleted: false,
        };
      },
    );

    return { accounts, count: accountReferral.count };
  }

  async getReferralStatus(account: Account) {
    const returnStatus: {
      referrerInfo: {
        name: string;
        avatarUrl: string;
    ...
    const accountReferrerInfo = await this.accountReferralRepository.findOne({
      where: {
      ...
      include: [
        {
          model: Account,
          as: 'Referrer',
          attributes: ['username', 'avatarUrl', 'follower'],
        },
      ],
    });

    const accountReferralTotalCount =
      await this.accountReferralRepository.count({
        where: {
          referrerId: account.id,
        },
      });
    ...
    return {
      referrerInfo: accountReferrerInfo?.Referrer
        ? {
            name: accountReferrerInfo?.Referrer?.username,
            avatarUrl: accountReferrerInfo?.Referrer?.avatarUrl ?? null,
            follower: accountReferrerInfo?.Referrer?.follower ?? 0,
          }
        : null,
      totalInvitees: accountReferralTotalCount,
    };
  }

  async getAccountReferrerId(accountId: string) {
    if (!accountId) {
      return null;
    }

    const accountReferrer = await this.accountReferralRepository.findOne({
    ...
    return accountReferrer?.referrerId;
  }

  async walletSummary(query) {
    const chain = +query.chain;
    const address = query.address?.toLowerCase() || '';
    const contract = query.contract?.toLowerCase() || '';
    const queueKey = CacheKeys.walletSummaryKey(address, contract, chain);
    const updateQueueStatus: AssetOwnersUpdateQueue | null | undefined =
      await this.cacheService.getCache(queueKey);
    const queueStatus = updateQueueStatus?.queueStatus || QUEUE_STATUS.PENDING;

    // get summary from db
    let lastSummary;
    if (!contract) {
      lastSummary = await this.walletSummaryRepository.findOne({
        where: { address: address, chain: chain },
        order: [['created_at', 'desc']],
      });
    } else {
      lastSummary = await this.walletNftSummaryRepository.findOne({
        where: {
          address: address,
          chain: chain,
    ...
    if (!updateQueueStatus) {
      await this.queueService.sendMessageToFifoSqs(
        this.configService.get(AWS_SQS_WALLET_SUMMARY_URL),
        {
          chain,
          address,
      ...
      await this.cacheService.setCache(
        ...
        this.configService.get(QUEUE_ENV.QUEUE_WALLET_SUMMARY_EXPIRED),
      );
    }
