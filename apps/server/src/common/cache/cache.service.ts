...
export class CacheKeys {
  ...
  static walletSummaryKey(address: string, contract?: string, chain?: number) {
    if (!contract) {
      return `aws-sqs-wallet-summary-url-${chain}-${address.toLowerCase()}`;
    } else {
      return `aws-sqs-wallet-summary-url-${chain}-${address.toLowerCase()}-${contract}`;
    }
  }

  static accountSummaryStatsKey(account: string) {
    return `aws-sqs-account-summary-url-${account}`;
  }

  static accountSummarySyncTaskKey() {
    return `aws-sqs-account-summary-url-accountSummarySyncTask`;
  }
